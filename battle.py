...
class Battle(Scene):
    def __init__(self, score, l_units, r_units, difficulty,
    ...
    def init(self):
        ...
        self.period_events = {1:[0, _("现在，开始进攻！"), lambda self: None],
                              2:[180000, _("倒计时{}秒").format("180"), lambda self:exec("self.money_got[1]*=2")],
                              3:[120000, _("倒计时{}秒").format("60"), lambda self:exec("self.money_got[1]*=1.5")],
                              4:[50000, _("倒计时{}秒").format("10"), lambda self: None],
                              5:[10000, "", lambda self:self.endmatch(_("时间到"))]}
        
        self.action_bar = load_image("action_bar.png").copy()
        self.action_bar_height = self.action_bar.get_height()
        self.cost_bar = pg.Surface((800, self.action_bar_height)).convert_alpha()
        ...
        for i in range(8):
            ...
            icon = get_unit_icon(self.l_units[i])
            ...
            if unit.unittype == "soldier":
                skill = sys.stdout.all_units[unit.en_name][-1]
                if skill: skill_list[0].append((unit.en_name, skill, "l"))
        ...
        for i in range(8):
            ...
            en_name = self.r_units[i]
            icon = get_unit_icon(en_name)
            icon_rect = pg.Rect((column*85+840, row*62+11, 65, 65))
            self.unit_tip_rects.append(icon_rect)
            self.action_bar.blit(icon, icon_rect)

            skill = sys.stdout.all_units[en_name][-1]
            if skill: skill_list[1].append((en_name, skill, "r"))
        ...
        for i in range(2):
            try:
                skill_list[i] = random.choice(skill_list[i])
            except IndexError:
                skill_list[i] = None

        self.r_skill = None 
        for args in skill_list:
            if not args:
                continue
            en_name, skill, camp = args
            cn_name, tip, cost, maxfrozen = skill
            icon = load_image(en_name + "_skill_icon.png")
            icon_rect = icon.get_rect(bottomleft=(420 if camp == "l" else 730, 130))
            self.action_bar.blit(icon, icon_rect)
            self.unit_tip_rects.append(icon_rect)
            if camp == "l":
                self.unit_rects.append(icon_rect)
                self.unit_icons.append(UnitIcon(en_name, icon_rect, skill=(int(cost), int(maxfrozen))))

            tip = renders([(cn_name, (255, 255, 255), 18),
                           (tip, (255, 255, 255), 13)], bg=(0, 0, 0, 150))
            self.unit_tips.append(tip)

            self.cost_bar.blit(render(str(cost), size=11), icon_rect.bottomright)

            if camp == "r":
                self.r_skill = [cost, eval("c_" + en_name + ".skill")]
    ...
    def show_tip(self, text, color=(0, 0, 255), size=36, keep=0, fadein=100, fadeout=100):
        self.tip_sprites.add(FadeOutText(text, (WIDTH//2, HEIGHT//2), color, size, keep, fadein, fadeout))
        
    def get_unit_tip(self, unit):
        tip = renders([(unit.cn_name, (255, 255, 255), 18),
                       (unit.DOC, (255, 255, 255), 13)], bg=(0, 0, 0, 150))
        return tip
    
    def take_damage(self, camp, damage):
        exec(f"self.{camp}_health -= damage")
        if eval(f"self.{camp}_health <= 0"):
            self.endmatch(_("攻破敌阵") if camp == "r" else "")
        else:
            self.update_pbar()
    ...
    def update_camera(self, value):
        self.camera = pg.math.clamp(self.camera+value, 0, self.camera_max)
        self.update_minimap()
    ...
    def update_ai(self):
        if random.random() < self.difficulty:
            if self.r_skill and random.random() < 0.2:
                if self.r_money >= self.r_skill[0]:
                    self.r_skill[1](self, "r")
                    self.r_money -= self.r_skill[0]
            else:       
                unit = random.choice(self.r_units)
                cost = sys.stdout.all_units[unit][2]
                if self.r_money >= cost:
                    try:
                        self.add_soldier(unit, "r", random.randint(0, self.maxroad))
                    except TypeError:
                        '''Test self.add_spell(unit, "r", '''
                    self.r_money -= cost
    ...
    def onexit(self):
        ...
        if self.timer > 5000:
            play_sound("defeat")
            LoseScene(self.draw_base, "l", ["", self.command_lose()])
            ...
            if event.button == 1:
                ...
                for rect in self.unit_rects:
                    if rect.collidepoint(event.pos):
                        ...
                        if self.unit_icons[idx].unittype == "skill":
                            unit_icon = self.unit_icons[idx]
                            if unit_icon.can_buy(self.l_money):
                                self.add_money("l", -unit_icon.cost)
                                eval(unit_icon.skill)(self, "l")
                                unit_icon.freeze()
                        else:
                    ...
                    if self.minimap_rect.collidepoint(event.pos):
                        self.camera = pg.math.clamp(event.pos[0] - self.minimap_rect.width / 4 - \
    ...
    def update(self):
        ...
        if self.next_period_to != False:
            if self.next_period_to == None:
                ...
                if now >= self.next_period_to:
                    self.period += 1
                    self.next_period_to = None
                    self.show_tip(self.period_events[self.period][1])
        ...
        for road in self.soldiers:
            for sprite in road:
                ...
                default_camp = sprite.camp
                player_projectile = self.l_projectile_group
                enemy_projectile = self.r_projectile_group
                if sprite.camp == "r":
                    player_projectile, enemy_projectile = enemy_projectile, player_projectile
                for projectile in enemy_projectile:
                    if projectile.col(sprite):
                        ...
                        if att:
                            ...
                            if projectile.maxtarget <= 0 and projectile.is_do_kill:
                                projectile.kill()
                                break
                            elif not projectile.is_do_kill:
                                projectile.is_do_kill = True
                                if projectile.camp == default_camp:
                                    player_projectile.add(projectile)
                                    enemy_projectile.remove(projectile)
                                break
